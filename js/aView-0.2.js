;(function($){
	$.fn.aView=function(settings){
		...
		if(_settings.mode=='tabviwe'){
			...
			return this.each(function(){$($(this).val()+" "+_settings.tab+" "+_settings.tabtype).on(_settings.bind,_settings.callback);});
		}
			...
			return this.each(function(){$($(this).val()+" "+_settings.tab).on(_settings.bind,_settings.callback);});
		}
		else if(_settings.mode=='layer'){
			$(_settings.content).hide();
			$(_settings.content).eq(0).show();
			return this.each(function(){$(this).on(_settings.bind,_settings.callback);});
		}
	};
})(jQuery);
